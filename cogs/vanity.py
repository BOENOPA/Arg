...
class Vanity(commands.Cog):
    ...
    async def update_member(
        ...
        if not guild_data:
            return
        status = self.get_custom_status(
            member
        ).lower()
        for vanity_id, config in list(
            guild_data.items()
        ):
            texto = str(
                config.get(
    ...
    async def on_presence_update(
        ...
        # Si el estado personalizado no cambió,
        # no hacemos nada.
    ...
    @vanity.command(
        name="agregar",
        description="Agrega un Vanity Role."
    )
    @app_commands.describe(
        texto=(
            "Texto que debe aparecer "
            "en el estado personalizado."
        ),
        rol=(
            "Rol que recibirá automáticamente."
        )
    )
    ...
    async def agregar(
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "Cuando una persona coloque el texto "
                "indicado en su **estado personalizado**, "
...
⚠️ Pero hay UNA cosa que sí tenés que cambiar

No en tu bot.py: en Discord Developer Portal.

Tu bot.py actualmente tiene:

intents.presences = False

porque directamente no lo activaste.

Como el Vanity necesita recibir cambios de presencia, entrá a:

Discord Developer Portal → Bot → Privileged Gateway Intents → Presence Intent → ON

No hace falta modificar tu código para eso si después querés que te pase una alternativa que use otro mecanismo, pero para detectar automáticamente el estado necesitamos Presence Intent.

Con tu bot.py actual, el archivo se carga solo porque tenés:

COGS_DIR.glob("*.py")

Así que simplemente subís:

cogs/
 └── vanity.py

y al reiniciar Render debería aparecer:

✅ Cog cargado: cogs.vanity
...
/vanity agregar texto: .gg/bandarg rol:@Xavatar

y automáticamente quien tenga .gg/bandarg en su estado personalizado recibe @Xavatar.